...
import { ArrowLeft, User, Package, MapPin, Truck, Shield, Calendar, CreditCard } from "lucide-react";

export const metadata: Metadata = { title: "Buchungsdetails" };

const STATUS_LABEL: Record<string, string> = {
  PENDING:"Anfrage eingegangen", CONFIRMED:"Bestätigt", ACTIVE:"Aktiv",
  DISPATCHED:"Versendet", DELIVERED:"Geliefert", RETURN_INITIATED:"Rückgabe angefragt",
  RETURN_DISPATCHED:"Rückversand", RETURNED:"Zurückgegeben",
  COMPLETED:"Abgeschlossen", CANCELLED:"Storniert",
};

// Timeline: ordered steps with their timestamp field name
const TIMELINE: Array<{ key: string; label: string; tsField: keyof typeof TIMESTAMP_FIELDS }> = [
...
// TypeScript trick: define which fields are timestamps so we can index safely
const TIMESTAMP_FIELDS = {
  createdAt: true, confirmedAt: true, dispatchedAt: true, deliveredAt: true,
  returnInitiatedAt: true, returnedAt: true, completedAt: true,
} as const;

type TsField = keyof typeof TIMESTAMP_FIELDS;
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-gray-900">Buchung #{booking.id.slice(-8).toUpperCase()}</h1>
        <Badge variant={bookingStatusVariant(booking.status)}>
          {STATUS_LABEL[booking.status] ?? booking.status}
        </Badge>
        <div className="ml-auto">
          <AdminBookingActions bookingId={booking.id} currentStatus={booking.status} showDelete />
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="md:col-span-2">
          <CardHeader><CardTitle className="flex items-center gap-2"><Calendar className="h-4 w-4" /> Statusverlauf</CardTitle></CardHeader>
          <CardContent>
                  ...
                  return (
                    <li key={step.key} className="flex items-start gap-4 pl-1">
                      <div className={[
                        ...
                        isCurrent ? "border-brand-400 bg-white" : "border-gray-300 bg-white",
                      ].join(" ")}>
                        {reached ? (
        ...
        {/* ── Renter ─────────────────────────────────────────────── */}
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><User className="h-4 w-4" /> Mieter</CardTitle></CardHeader>
        ...
        {/* ── Vendor ─────────────────────────────────────────────── */}
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Package className="h-4 w-4" /> Vermieter & Produkt</CardTitle></CardHeader>
          <CardContent className="space-y-2 text-sm">
            <p className="font-medium text-gray-900">{booking.product.vendor.name}</p>
            {booking.product.vendor.company && <p className="text-gray-500">{booking.product.vendor.company}</p>}
            <p className="text-gray-500">{booking.product.vendor.email}</p>
            <hr className="my-2" />
            <div className="flex items-center gap-2">
              {booking.product.images[0] && (
                <img src={booking.product.images[0]} alt="" className="h-10 w-10 rounded-lg object-cover" />
              )}
              <div>
        ...
        {/* ── Booking dates ───────────────────────────────────────── */}
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Calendar className="h-4 w-4" /> Mietdaten</CardTitle></CardHeader>
          <CardContent className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{rentalDays} Tag{rentalDays !== 1 ? "e" : ""}</span>
            </div>
            <div className="flex justify-between">
        ...
        {/* ── Pricing breakdown ───────────────────────────────────── */}
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><CreditCard className="h-4 w-4" /> Kostenaufschlüsselung</CardTitle></CardHeader>
          <CardContent className="space-y-2 text-sm">
            <div className="flex justify-between text-gray-600">
              <span>Miete ({rentalDays} Tage × {booking.quantity} Stk.)</span>
              <span>{formatCurrency(Number(booking.totalPrice) - Number(booking.transportCost) - Number(booking.insuranceCost))}</span>
            </div>
            {booking.transportIncluded && (
              <div className="flex items-center justify-between text-gray-600">
                <span className="flex items-center gap-1"><Truck className="h-3.5 w-3.5" /> Transport</span>
                <span>{formatCurrency(Number(booking.transportCost))}</span>
              </div>
            )}
            {booking.insuranceIncluded && (
              <div className="flex items-center justify-between text-gray-600">
                <span className="flex items-center gap-1"><Shield className="h-3.5 w-3.5" /> Versicherung</span>
            ...
            <div className="flex justify-between text-xs">
              ...
              <span className={booking.depositPaid ? "text-green-600 font-medium" : "text-yellow-600"}>
                {booking.depositPaid ? "Anzahlung erhalten" : "Ausstehend (Stripe)"}
              </span>
            </div>
          </CardContent>
        </Card>
