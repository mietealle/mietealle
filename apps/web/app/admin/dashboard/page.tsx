...
import { Users, Package, CalendarCheck, TrendingUp, AlertTriangle } from "lucide-react";

export const metadata: Metadata = { title: "Admin Dashboard" };

export default async function AdminDashboard() {
  const [userCount, productCount, bookingCount, pendingVendors, revenueResult, recentBookings] = await Promise.all([
    prisma.user.count(),
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Link href="/admin/users" className="text-xs text-brand-600 hover:underline">Alle anzeigen →</Link>
            </div>
          </CardHeader>
          <CardContent>
            {pendingVendors.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {pendingVendors.map((v) => (
                  <div key={v.id} className="flex items-center justify-between rounded-lg bg-yellow-50 p-3">
                    <div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Link href="/admin/bookings" className="text-xs text-brand-600 hover:underline">Alle →</Link>
            </div>
          </CardHeader>
          <CardContent>
