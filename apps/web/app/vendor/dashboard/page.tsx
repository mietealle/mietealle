...
export default async function VendorDashboard() {
  ...
  const [vendor, productCount, bookings] = await Promise.all([
    prisma.user.findUnique({ where: { id: vendorId }, select: { commissionRate: true, verificationStatus: true, name: true } }),
    prisma.product.count({ where: { vendorId } }),
    prisma.booking.findMany({
      where: { product: { vendorId } },
      select: { totalPrice: true, status: true, transportCost: true, insuranceCost: true },
    }),
  ]);
  ...
  const activeBookings = bookings.filter((b) => ["ACTIVE", "CONFIRMED", "DISPATCHED", "DELIVERED"].includes(b.status)).length;
